...
#   Seiichirou Hiraoka <seiichirou.hiraoka@gmail.com> with ChatGPT 4o with GitHub Copilot
...
def aggregate_data(data, interval_minutes):
    ...
    # インターバルを設定
    grouped = grouped.reindex(intervals, fill_value=0)
    # インデックスを設定
...
def create_stackplot(df, pdf, show_legend):
    ...
    # 積み上げ棒グラフを作成
    # 縦軸を対数表示に設定
    bars = df.plot(kind='bar', stacked=True, ax=ax, edgecolor='black', logy=True)
    # タイトルを設定
